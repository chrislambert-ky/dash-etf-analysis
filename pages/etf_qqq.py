...
# Download and process data for QQQ
df = download_stock_data("QQQ")
...
# Layout with graph and table for QQQ
layout = html.Div([
    html.H1("QQQ ETF Analysis"),

    # Graph for QQQ with pattern-matching ID
    dcc.Graph(
        id={'type': 'stock-graph', 'index': 'qqq'},
        figure=create_stock_graph(df, purchased, forecast, linear_forecast)
    ),

    # Table for QQQ purchases with pattern-matching ID
    dash_table.DataTable(
        id={'type': 'purchases-table', 'index': 'qqq'},
        columns=[
            {"name": "Date", "id": "Date"},
