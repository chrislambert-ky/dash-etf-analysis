...
# Download and process data for SPY
...
# Layout with graph and table for SPY
layout = html.Div([
    html.H1("SPY ETF Analysis"),

    # Graph for SPY with pattern-matching ID
    dcc.Graph(
        id={'type': 'stock-graph', 'index': 'spy'},
        figure=create_stock_graph(df, purchased, forecast, linear_forecast)
    ),

    # Table for SPY purchases with pattern-matching ID
    dash_table.DataTable(
        id={'type': 'purchases-table', 'index': 'spy'},
        columns=[
            {"name": "Date", "id": "Date"},
